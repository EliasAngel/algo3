...
public enum Direccion {
    ...
    public boolean noEstaEnLosLimites(int posicionX, int posicionY, int cantFilas, int cantColumnas) {
        ...
        if (posicionX + movimientoFila < -1 || posicionX + this.movimientoFila > cantFilas) {
            sePuedeMoverEnX = false;
        }
        if (posicionY + movimientoColumna < -1 || posicionY + this.movimientoColumna > cantColumnas) {
            sePuedeMoverEnY = false;
        }
